...
class m_admin extends Database {
    ...
    public function get_laporan_aktif($filter = []) {
        ...
        // Kita filter agar data yang sudah masuk ke tb_riwayat TIDAK DITAMPILKAN LAGI
        $where = "WHERE t1.id_pelaporan NOT IN (SELECT id_pelaporan FROM tb_riwayat)";

        // Filter tambahan jika Admin menggunakan fitur pencarian
        if (!empty($filter['tgl_awal']) && !empty($filter['tgl_akhir'])) {
            $where .= " AND t1.tgl_laporan BETWEEN '{$filter['tgl_awal']}' AND '{$filter['tgl_akhir']}'";
        }
        if (!empty($filter['nama'])) {
            $where .= " AND s.nama LIKE '%{$filter['nama']}%'";
        }
        ...
        $query = "SELECT 
                  ...
                  JOIN tb_siswa s ON t1.nis = s.nis
                  JOIN tb_kategori k ON t1.id_kategori = k.id_kategori
                  $where
                  ORDER BY t1.tgl_laporan DESC";
    ...
    public function get_riwayat_filter($filter = []) {
        ...
        // INI KODE YANG MEMBUAT FILTER KATEGORI BERFUNGSI KEMBALI
    ...
    public function proses_tanggapan($id_pelaporan, $status_baru, $feedback, $id_admin) {
